...
bool InitializeMetadataWalkFrame(
    MetadataWalkFrame* frame,
  ...
  if (!frame->target->metadata().WalkStep(
          build_settings, keys_to_extract, keys_to_walk, rebase_dir,
          &next_walk_keys, &frame->current_result, err)) {
    return false;
  }
  ...
  for (const auto& next : next_walk_keys) {
    ...
    Label next_label =
        Label::Resolve(current_dir, build_settings->root_path_utf8(),
  ...
  while (!stack.empty()) {
    ...
    while (frame.next_visit_index < frame.targets_to_visit.size()) {
      const Target* next_target =
          frame.targets_to_visit[frame.next_visit_index++];
