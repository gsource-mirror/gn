...
bool Target::OnResolvedWithoutChecks(Err* err) {
  ...
  for (const auto& dep : GetDeps(DEPS_LINKED)) {
    ...
    if (t->module_type().test(MODULEMAP_INHERITED_TEXTUAL)) {
      ...
      if (module_type_.test(HAS_MODULEMAP) && !module_type_.test(MODULEMAP_EXACT)) {
  ...
  if (module_type_.test(HAS_MODULEMAP) && module_type_.test(MODULEMAP_INHERITED_TEXTUAL) && !module_type_.test(MODULEMAP_IS_TEXTUAL)) {
    std::ostringstream os;
    // Try is the safest option, since it gives clear error messages.
    os << "To modularize targets, add `generate_modulemap = "
          "\"try|inherit|textual\"` (prefer \"try\") to all "
          "non-modularized targets it transitively depends on:\n";
