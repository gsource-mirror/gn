...
std::vector<const Target*> FindDependencyPath(const Target* from,
                                              const Target* to) {
  // Use a vector for performance since it's contiguious, and the maximum
  // queue size should trivially fit in memory.
  std::vector<const Target*> queue;
  std::unordered_map<const Target*, const Target*> parents;
  parents[from] = nullptr;

  size_t upto = 0;
  const Target* cur = from;
  do {
  ...
  } while (upto < queue.size() && (cur = queue[upto++]) != to);
  ...
  auto OutputDepSuggestion = [&](std::vector<const Target*> candidates) {
    ...
    std::sort(labels.begin(), labels.end(),
              [](const std::string& lhs, const std::string& rhs) {
                ...
                return std::make_pair(!lhs.starts_with(':'), lhs) <
                       std::make_pair(!rhs.starts_with(':'), rhs);
              });
