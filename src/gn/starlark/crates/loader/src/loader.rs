...
impl FileLoader {
    ...
    ) -> starlark::Result<&'a FrozenModule> {
    ...
    fn get_loaded<'a>(&'a self, status: &FileStatus) -> starlark::Result<&'a FrozenModule> {
        match status {
            FileStatus::Loading { .. } => unreachable!(),
            // Safety:
            // 1. The `FrozenModule` is wrapped in `Pin<Box<...>>`, meaning it is heap-allocated and
            //    its memory address is stable even if the parent `HashMap` reallocates.
            // 2. The `FileLoader` cache is insert-only (entries are never removed, cleared, or
            //    overwritten), so the heap memory remains valid for the entire lifetime `'a` of
            //    `FileLoader`.
            // 3. Although we access this outside the `MutexGuard` lock, the module is frozen and
            //    immutable once loaded, so no concurrent data races can occur.
            FileStatus::Loaded(Ok(m)) => Ok(unsafe {
                std::mem::transmute::<&FrozenModule, &'a FrozenModule>(m.as_ref().get_ref())
            }),
    ...
    pub fn load<'a, 'b, C: EvalContext, F: Fn(&PackageRef) -> Box<C>>(
        &'a self,
        label: LabelRef<'b>,
        ...
        let content =
            fs::read_to_string(&absolute_path).map_err(|_| Error::ReadFailed(label_str.clone()))?;
        ...
        let module = Module::with_temp_heap(|module| {
            ...
            Ok(module.freeze_named(FrozenHeapName::User(Box::new(label_str.clone())))?)
        });
...
mod tests {
    ...
    fn load<'a>(loader: &'a FileLoader, label_str: &str) -> starlark::Result<&'a FrozenModule> {
        loader.load(
