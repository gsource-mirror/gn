...
ResolvedTargetData::TargetInfo* ResolvedTargetData::GetTargetInfo(
  ...
  // to this ResolvedTargetData instance's generation. If it matches, the pointer
  // is valid and we can return it directly without taking locks or computing hashes.
...
ResolvedTargetData::TargetInfo* ResolvedTargetData::GetTargetInfoSlow(
  ...
  // concurrently, so serialize under infos_mutex_ and double-check the generation.
  ...
  // with memory_order_acquire that sees 'generation == generation_' will observe
  // the valid result pointer.
