...
SuggestResult OutputSuggestions(const std::vector<const Target*>& all_targets,
  ...
  if (visible_candidates.size() > 1) {
    auto ClosenessHeuristic = [&](const Target* candidate) -> size_t {
      ...
      //   visible to //third_party/absl, but may not be.
      //
      // Then:
      ...
      //   (//absl:absl).
      const std::string& cand_dir = candidate->label().dir().value();
      const std::string& includer_dir = includer->label().dir().value();
      const std::string& included_dir = included->label().dir().value();
      ...
      if (includer_dir.starts_with(cand_dir)) {
        ...
        // original target. Eg. //absl re-exporting //absl/subdir:foo We
        // intentionally make no decision on whether //absl:exporter or
