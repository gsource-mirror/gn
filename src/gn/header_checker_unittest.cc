...
TEST_F(HeaderCheckerTest, CheckIncludesStrictTransitive) {
  ...
  EXPECT_TRUE(errors[0].help_text().find("check_includes_strict = true") !=
              std::string::npos)
      << "Actual help text:\n"
      << errors[0].help_text();
  EXPECT_TRUE(errors[0].help_text().find(
                  "Including headers from the public_deps of //b:b "
                  "is blocked because check_includes_strict = true is enabled "
                  "on it.") != std::string::npos)
      << "Actual help text:\n"
      << errors[0].help_text();
  EXPECT_TRUE(errors[0].help_text().find(
...
TEST_F(HeaderCheckerTest, CheckIncludesStrictSameTargetPrivateHeader) {
  ...
  ASSERT_EQ(errors.size(), 1u);
  EXPECT_TRUE(errors[0].message().find("Public headers cannot include private "
  ...
  EXPECT_TRUE(errors[0].help_text().find(
                  ...
                  "add it to public_deps") != std::string::npos)
      << "Actual help text:\n"
      << errors[0].help_text();
