...
TEST(Suggest, StandardTargetExposes) {
  TestWithScope setup_scope;
  Err err;
  Label default_toolchain = setup_scope.toolchain()->label();
  ...
  auto create_standard_target_in_dir = [&](std::string_view name,
                                           const SourceDir& dir, auto fn) {
    auto target = std::make_unique<Target>(
        setup_scope.settings(),
    ...
    target->sources().push_back(
        SourceFile(dir.value() + std::string(name) + ".cc"));
    fn(target.get());
    Err err;
    EXPECT_TRUE(target->OnResolvedWithoutChecks(&err));
    all_targets.push_back(target.get());
  ...
  auto b = create_standard_target_in_dir(
      "b", SourceDir("//sub/"), [&](Target* t) { set_visibility(t, "//:a"); });
  auto a = create_standard_target_in_dir("a", SourceDir("//"), [&](Target* t) {
    t->public_deps().push_back(LabelTargetPair(b.get()));
    t->visibility().SetPublic();
  });
  auto includer = create_standard_target_in_dir("includer", SourceDir("//"),
                                                [](Target* t) {});
  ...
  commands::OutputSuggestions(all_targets, setup_scope.build_settings(),
                              default_toolchain, "//:includer", "//sub:b",
                              collect);

  EXPECT_EQ(
      "Warning: //sub:b is not visible to //:includer\n"
      "Suggestion: Carefully consider whether you want to change the "
      "visibility so that you can depend on it\n"
      "Suggestion: Add public_deps = [ \"//sub:b\" ] to :includer (defined at "
      "//BUILD.gn:1)\n",
      output);
}
