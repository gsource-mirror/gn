...
TEST(SessionTest, SessionLoad) {
  ...
  std::vector<std::string_view> keys = {"absolute_value", "relative_value"};

  Err err;
  bool success = session_load(*session, ":root.bzl", SourceDir("//load/"), keys,
                              *setup.scope(), ParseNodePtr{.ptr = nullptr}, err);
  EXPECT_TRUE(success);
