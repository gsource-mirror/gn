...
// We can't just use Pretty(bool value) because then it takes in any type
// convertible to bool instead of specifically bools.
template <typename T>
...
std::string_view Pretty(const T& value) {
...
// Pointer should come first because we pretty much universally point to fixed
// types rather than templates.
template <typename T>
...
std::string TryDiffStrings(const T& expected, const U& actual) {
  if constexpr (requires {
    ...
    if (!expected_pretty.ends_with("\n") && !actual_pretty.ends_with("\n")) {
      return DiffStrings(std::string(expected_pretty) + "\n",
                         std::string(actual_pretty) + "\n");
    }
