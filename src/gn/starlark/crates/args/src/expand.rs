...
use starlark::{
    eval::Evaluator,
    values::{list::ListRef, tuple::TupleRef, FrozenValue, Value, ValueLike},
};
...
/// Helper to process and append a specific `ArgsGen` object to the action's
/// command line.
pub fn expand_into<'v>(
    command: &mut Vec<String>,
    // Because:
    // * expand_into requires a borrow
    // * garbage collection requires a mutable borrow
    // * map_each means that we can continue running code, thus could trigger garbage collection.
    // It is not safe to expand a non-frozen ArgsGen. This is perfectly fine though, because we
    // never actually want to expand a non-frozen one.
    args_obj: &ArgsGen<FrozenValue>,
