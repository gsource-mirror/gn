...
pub struct ProviderType {
    ...
    /// * A unique identifier for the current module, allowing you to determine
    ///   whether self is mutable.
    ///   * This will allow you to determine whether it's safe to fill the
    ///     metadata.
    /// * The metadata about providers that requires knowing the name to
    ///   implement.
    pub(crate) data: UnsafeCell<Either<usize, ProviderTypeData>>,
...
// Since freezing errors out if data is not initialized, once successfully
// frozen data is guaranteed to be Some(...) and read-only, making concurrent
// reads safe.
...
impl<'v> StarlarkValue<'v> for ProviderType {
    ...
    fn invoke(
        ...
        let spec = match unsafe { &*self.data.get() } {
            Either::Right(data) => &data.parameter_spec,
            Either::Left(_) => return Err(Error::ProviderNotExported.into()),
        };
    ...
    fn export_as(&self, name: &str, eval: &mut Evaluator<'v, '_, '_>) -> starlark::Result<()> {
        // Safety: Shared read is safe. If it is already Either::Right, we exit early
        // without any mutations or mutable references.
        let data_opt = unsafe { &*self.data.get() };
        match data_opt {
            Either::Left(creator_id) => {
                // Our objective here is to check whether &self is frozen.
                // Frozen types are immutable and thus we cannot safely (and do not want to)
                // export it.
                //
                // We use the fact that iff we're in a different module than the one where
                // self was created, then self must be frozen.
                if get_module_id(eval.module()) != *creator_id {
                    return Ok(());
                }
                ...
                // Safety: Since creator_id matches, we are during the single-threaded
                // evaluation of the creator module, so mutation is safe.
                unsafe {
                    *self.data.get() = Either::Right(data);
                }
            },
            Either::Right(_) => {},
        }
...
impl ProviderType {
    ...
    /// is called.
    pub fn new(fields: Vec<String>, module: &Module) -> starlark::Result<Self> {
        ...
        Ok(Self {
            ...
            data: UnsafeCell::new(Either::Left(get_module_id(module))),
            fields: field_map,
        })
    }
    ...
    pub(crate) fn configured(&self) -> &ProviderTypeData {
        ...
        unsafe {
            match &*self.data.get() {
                ...
                Either::Left(_) => std::hint::unreachable_unchecked(),
            }
        }
    }
}
...
"#,
    ...
    fn test_provider_aliasing() {
        ...
        let p_info = a.pass("Info = provider(fields = ['a']); Info");
        a.modify_globals(move |builder| {
            builder.set("PInfo", p_info.clone());
        });

        let alias = a.pass("alias = PInfo; alias");
        a.modify_globals(move |builder| {
            builder.set("alias", alias.clone());
        });

        // The constructor should retain its canonical name "PInfo"
        a.eq("str(alias(a = 1))", "Info(a = 1)".to_string());
    }

    #[test]
    fn test_unexported_provider_fails_to_export_in_different_module() {
        let mut a = new_assert();
