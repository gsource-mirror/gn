...
class NinjaFile:
  ...
  def __init__(self, platform, source_root, out_dir):
    ...
    def python(path, args):
      return (
          ...
          f' {shlex.quote(str(self.source_file(path)))} {args}'
      )
  ...
  def write_ninja(self):
    ...
    for action in self.actions:
      inputs = ' '.join([shlex.quote(str(p)) for p in action.inputs])
      inputs = f' {inputs}' if inputs else ''

      implicit = ' '.join([shlex.quote(str(p)) for p in action.implicit_inputs])
