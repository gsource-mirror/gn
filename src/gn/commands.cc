...
std::optional<int> TryRunCommandInServer(const std::string& command,
  ...
  auto client = util::ClientSocket::Connect(port);
  if (!client) {
    *err = Err(Location(),
        "Could not connect to GN server on port " + base::NumberToString(port));
    return std::nullopt;
  }
  ...
  while (true) {
    ...
    switch (kind) {
      ...
      case ServerProtocol::kPrint:
        if (auto str = util::DeserializeString(span); str) {
          OutputString(*str);
        } else {
          *err = Err(Location(), "Failed to deserialize print string");
          return std::nullopt;
        }
        break;
      ...
      default:
        *err = Err(Location(), "Unknown message kind " + base::NumberToString(kind) +
                            " from GN server.");
        return std::nullopt;
    }
  }
