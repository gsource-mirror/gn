...
// target.
std::optional<bool> SourceIsPrivate(const Target* target,
...
bool AddToolchainSources(const std::vector<const Target*>& all_targets,
  ...
  for (const Target* target : all_targets) {
    if (!current_toolchain ||
      ...
      if (auto is_private = SourceIsPrivate(target, file);
...
bool OutputSuggestions(const std::vector<const Target*>& all_targets,
  ...
  const auto& [includer, source_private] = includer_targets.front();
  ...
  const auto& [included, included_is_private] = targets.front();
  ...
  // * Check if included transitively depends on included. Suggest ways to break
  // the loop.
