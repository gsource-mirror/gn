...
TEST(NinjaCreateBundleTargetWriter, PostProcessing) {
  ...
  const char expected[] =
      ...
      "  restat = 1\n"
      "build phony/baz/bar.inputdeps: phony || ./quz phony/foo/bar phony/foo/data\n"
      ...
      "build phony/baz/bar.postprocessing.inputdeps: phony ../../build/codesign.py quz bar.bundle/Contents/Resources/input1.txt bar.bundle/Contents/Resources/input2.txt | phony/baz/bar.inputdeps\n"
...
TEST(NinjaCreateBundleTargetWriter, PostProcessingNoStampFilesCustomToolchain) {
  ...
  const char expected[] =
      ...
      "  description = POST PROCESSING //baz:bar(//toolchain:default)\n"
      "  restat = 1\n"
      "build toolchain/phony/baz/bar.inputdeps: phony || ./quz toolchain/phony/foo/bar toolchain/phony/foo/data\n"
