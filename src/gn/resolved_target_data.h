...
class ResolvedTargetData {
 ...
 private:
  ...
  // We shard the TargetInfo map to reduce lock contention under the high-concurrency
  // parallel writing phase of 'gn gen'.
  // 128 shards is chosen as the sweet spot based on benchmarking, providing optimal
  // scaling for high-core workstation counts (up to 128 threads) with negligible memory
  ...
  // We use std::hash to distribute targets evenly across shards and avoid pointer
  // alignment biases.
