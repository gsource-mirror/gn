...
Result<std::string> DoEdit(std::string command, std::string before) {
  return DoEdit(std::move(command), {"//:*"}, std::move(before));
}
...
TEST_F(EditCommandTest, MultipleTargetsSubset) {
  ASSERT_SUCCESS(DoEdit("set testonly true", {"//:foo"},
                        "executable(\"foo\") {\n"
                        "  testonly = false\n"
                        "}\n"
                        "executable(\"bar\") {\n"
...
TEST_F(EditCommandTest, SetSubcommand) {
  // New bool attribute
  ASSERT_SUCCESS(DoEdit("set testonly true",
                        "executable(\"foo\") {\n"
                        "}\n"),
                 Edited("executable(\"foo\") {\n"
                        "  testonly = true\n"
  ...
  // String attribute
  ASSERT_SUCCESS(DoEdit("set label \"//foo:bar\"",
                        "executable(\"foo\") {\n"
                        "}\n"),
                 Edited("executable(\"foo\") {\n"
                        "  label = \"//foo:bar\"\n"
  ...
  // Int attribute
  ASSERT_SUCCESS(DoEdit("set assert_no_deps 42",
                        "executable(\"foo\") {\n"
                        "}\n"),
                 Edited("executable(\"foo\") {\n"
                        "  assert_no_deps = 42\n"
