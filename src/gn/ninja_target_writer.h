...
struct CommonVars {
  std::map<const Substitution*, std::string> vars;

  std::string_view Get(const Substitution* subst) const {
    auto it = vars.find(subst);
    if (it != vars.end())
      return it->second;
    return {};
  }
...
class NinjaTargetWriter {
 public:
  ...
  void SetCommonVars(const CommonVars* common_vars) { common_vars_ = common_vars; }
  std::string_view GetCommonVar(const Substitution* subst) const {
    return common_vars_ ? common_vars_->Get(subst) : std::string_view();
  }
  ...
  static std::string RunAndWriteFile(
      ...
      const CommonVars* common_vars = nullptr);

  // Helper functions to pre-compute substitution strings for common variable hoisting.
  static std::string ComputeDefines(const Target* target);
  static std::string ComputeIncludeDirs(const Target* target,
                                        PathOutput& path_output);
  static std::string ComputeCFlags(const Target* target,
                                   const Substitution* subst,
                                   PathOutput& path_output);
  static std::string ComputeRustFlags(const Target* target,
                                      PathOutput& path_output);
 ...
 protected:
  ...
  // Common variables hoisted for the current BUILD.ninja file.
  const CommonVars* common_vars_ = nullptr;
