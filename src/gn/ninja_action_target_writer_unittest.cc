...
TEST(NinjaActionTargetWriter, ForEach) {
  ...
  const char expected_linux[] =
      ...
      "rule __foo_bar___rule\n"
      "  command = /usr/bin/python ../../foo/script.py -i ${in} --out=foo\\$ "
      "bar${source_name_part}.o\n"
      "  description = ACTION //foo:bar()\n"
      "  restat = 1\n"
      "build phony/foo/bar.inputdeps: phony ../../foo/script.py "
      "../../foo/included.txt phony/foo/dep\n"
      "build input1.out: __foo_bar___rule ../../foo/input1.txt | "
      "phony/foo/bar.inputdeps || phony/foo/bundle_data_dep phony/foo/datadep\n"
...
TEST(NinjaActionTargetWriter, ActionWithSpaces) {
  ...
  const char expected[] =
      ...
      "rule __foo_bar___rule\n"
      "  command = /Program\\$ Files/python ../../foo/my\\$ script.py my\\$ "
      "argument\n"
      "  description = ACTION //foo:bar()\n"
      "  restat = 1\n"
      "build foo.out: __foo_bar___rule | ../../foo/my$ script.py "
      "../../foo/input$ file.txt\n"
      "build phony/foo/bar: phony foo.out\n";
  EXPECT_EQ(expected, out.str()) << expected << "--" << out.str();
}
