...
// Inline it to ensure that the error doesn't actually need to be passed as a function parameter.
#[inline]
    ...
    if let Some(frozen) = this.downcast_ref::<ArgsGen<starlark::values::FrozenValue>>() {
        let mutable_args = ArgsGen {
            arguments: RefCell::new(
                frozen.arguments.borrow().iter().map(|arg| arg.to_value()).collect(),
            ),
        };
