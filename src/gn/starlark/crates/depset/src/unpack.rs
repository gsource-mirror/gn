...
use starlark::values::{FrozenValueTyped, UnpackValue, Value, ValueTyped};
use types::File;

use crate::depset::{Depset, FrozenDepset};

/// Helper type to unpack a Starlark `Value` as either a mutable or frozen `Depset`.
...
/// Helper to unpack a depset and retrieve its single `File` if it represents a phony file depset.
#[derive(Debug)]
pub struct UnpackFileDepset<'v>(pub Option<&'v File>);

impl StarlarkTypeRepr for UnpackFileDepset<'_> {
    type Canonical = UnpackFileDepset<'static>;
    fn starlark_type_repr() -> Ty {
        Ty::any()
    }
}

impl<'v> UnpackValue<'v> for UnpackFileDepset<'v> {
    type Error = starlark::Error;

    fn unpack_value_impl(value: Value<'v>) -> Result<Option<Self>, Self::Error> {
        let depset = UnpackDepset::unpack_value_err(value)?;
        if depset.phony().is_some() || depset.is_empty() {
            Ok(Some(UnpackFileDepset(depset.depset.phony().as_ref())))
        } else {
