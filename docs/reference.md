...
```
  ...
  In cases of ambiguity (eg. conditionals, +=, -=), `gn edit` will make
  a best effort attempt to do the right thing, but will leave notes
  in your build files instructing you what to do.
  eg. If gn edit suspects a line should be removed, it might add:
      # TODO(gn edit): double check the following line is safe to remove
```
...
```
  set <attribute> <value>
      ...
      Note: only bool, int, and string values are supported.
```
