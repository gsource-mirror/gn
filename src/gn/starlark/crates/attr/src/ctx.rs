...
impl CtxAttrSchema {
    ...
    pub fn new(
        ...
        let (builtin_fields, builtin_num_files) = builtin.map(|b| b.fields()).unwrap_or_default();
        let mut attrs_fields = SmallMap::with_capacity(attrs.len() + builtin_fields.len());
        ...
        for (i, name) in builtin_fields.iter().enumerate() {
            attrs_fields.insert(name.to_string(), any());
            if i < builtin_num_files {
    ...
    pub fn create_ctx_fields<'v, S: Session>(
        ...
        let (builtin_fields, builtin_num_files) = builtin.map(|b| b.fields()).unwrap_or_default();
        debug_assert!(builtin_fields.len() == ctx_attr.len());
        ctx_attr.reserve_exact(self.attr.len());
        let mut ctx_files = Vec::with_capacity(self.files.len() + builtin_num_files);
        for &attr_val in ctx_attr.iter().take(builtin_num_files) {
