...
impl<'v, 'a, 'e> EvaluatorContextExt<'v, 'a, 'e> for starlark::eval::Evaluator<'v, 'a, 'e> {
    ...
    fn context_mut<C: EvalContext>(&mut self) -> &mut C {
        ...
        let val_ref = unsafe { extra.unwrap_unchecked() }
            .downcast_ref::<C>()
            .unwrap();
        // Safety: We know that context is not borrowed already, either
        // mutably or immutably, because that would require a second
        // borrow of the evaluator itself while it's borrowed immutably.
