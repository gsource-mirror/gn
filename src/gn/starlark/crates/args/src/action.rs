...
use crate::args::Formatter;
use crate::errors::Error;

/// Represents a build action containing the parsed command-line arguments, input files, and output files.
#[derive(Default)]
pub struct Action {
    /// The command arguments to execute.
    pub command: Vec<String>,
    /// The list of input files detected from parsing the arguments.
    pub inputs: SmallSet<File>,
    /// The output files produced by this action.
    pub outputs: Vec<File>,
}

impl Action {
    /// Parses Starlark values representing arguments (which can be strings or `Args` objects) and appends them to the action's command line.
    pub fn add_args<'v>(
        &mut self,
        args: Value<'v>,
        ...
        for arg in args
            .iterate(eval.heap())
            .map_err(|_| Error::IterableRequired)?
        {
            if let Some(s) = arg.unpack_str() {
                self.command.push(s.to_owned());
            } else if let Some(args_obj) = arg.downcast_ref::<ArgsGen<Value<'v>>>() {
                self.add_arg(args_obj, eval)?;
            } else if let Some(args_obj) = arg.downcast_ref::<ArgsGen<FrozenValue>>() {
                self.add_arg(args_obj, eval)?;
            } else {
    ...
    /// Helper to process and append a specific `ArgsGen` object to the action.
    pub fn add_arg<'v, V: ValueLike<'v> + Copy>(
        &mut self,
        args_obj: &ArgsGen<V>,
        eval: &mut Evaluator<'v, '_, '_>,
    ) -> starlark::Result<()>
        ...
        F: FnOnce(&mut Vec<String>),
    {
        ...
        let has_items = Self::for_each(value, |v| {
