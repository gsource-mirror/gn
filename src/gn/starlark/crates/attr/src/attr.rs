    ...
    pub fn create_without_defaults(
            ...
            items
                ...
                .map(|s| {
                    ...
                    if !unique.insert(lf.clone()) {
                        return Err(crate::Error::DuplicateLabel(lf, param_name.to_owned()).into());
                    }
                    Ok(lf)
                })
                .collect()
        ...
        match &schema.kind {
            ...
            AttrKind::Int { allowed } => {
                let v = i64::unpack_named_param(value, param_name)?;
            ...
            AttrKind::IntList => {
                let list = UnpackList::<i64>::unpack_named_param(value, param_name)?;
