...
bool FormatJsonToString(const std::string& json, size_t format_width, std::string* output) {
...
bool FormatJsonToString(const std::string& json, std::string* output) {
  return FormatJsonToString(json, kDefaultFormatWidth, output);
}
...
bool FormatStringToString(const std::string& input,
                          ...
                          std::string* output,
                          std::string* dump_output) {
  return FormatStringToString(input, dump_tree, kDefaultFormatWidth, output, dump_output);
}
...
int RunFormat(const std::vector<std::string>& args) {
  ...
  if (from_stdin) {
    ...
    if (cmdline->HasSwitch(switches::kFormatWidth)) {
      ...
      if (!base::StringToInt(cmdline->GetSwitchValueString(switches::kFormatWidth), &format_width_int) || format_width_int <= 0) {
        Err(Location(), "Invalid --max-format-width value: " + format_width_str);
        return 1;
      }
    ...
    if (!FormatStringToString(input, dump_tree, format_width, &output, &dump_output))
      return 1;
  ...
  size_t format_width = setup.build_settings().format_width();

  if (cmdline->HasSwitch(kSwitchReadTree)) {
    std::string tree_type = cmdline->GetSwitchValueString(
            kSwitchReadTree);
    if (tree_type != kSwitchTreeTypeJSON) {
  ...
  for (const auto& arg : args) {
    ...
    if (!FormatStringToString(original_contents, dump_tree, format_width, &output_string,
                              &dump_output_string)) {
      exit_code = 1;
      continue;
    }
