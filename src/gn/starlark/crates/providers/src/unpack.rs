...
use crate::provider_instance::{FrozenProviderInstance, ProviderInstance};
...
impl StarlarkTypeRepr for UnpackProviderInstance<'_> {
    ...
    fn starlark_type_repr() -> Ty {
        // Technically we're unpacking a provider instance, but the message for the user
        // should be "expected provider", not "expected provider_instance"
        Ty::starlark_value::<crate::ProviderType>()
    }
}
...
mod tests {
    ...
    fn test_unpack_provider_instance() {
        ...
        assert_eq!(
            err.to_string(),
            "Type of parameters mismatch, expected `provider`, actual `int (repr: 1)`"
        );
