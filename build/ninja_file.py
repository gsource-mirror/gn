...
class NinjaFile:
  ...
  def __init__(self, platform, source_root, out_dir, debug=False):
    ...
    symlink_rel_path = 'build/symlink_cargo_output.py'
    symlink_script = self.source_file(symlink_rel_path)
    self.Cargo = Rule(
        name='cargo',
        ninja_file=self,
        command=self.chain(
            f'env CXX=$cxx NINJA_OUT_DIR={self.out_dir} RUSTFLAGS="-L {self.out_dir}"'
            ' $cargo_env cargo build --color=always'
            ' --manifest-path=$manifest_path $cargo_target_dir $cargo_flags' + ('' if self.debug else ' --release'),
            python(
                symlink_rel_path,
                '$target_type $out $cargo_out_dir',
            ),
        ),
