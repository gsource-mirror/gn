...
void NinjaCBinaryTargetWriter::WriteModuleNameSubstitution() {
  if (target_->toolchain()->substitution_bits().used.count(
    ...
    target_group_.target_vars.push_back(
        {CSubstitutionModuleName.ninja_name, val.str()});
  }
}
...
void NinjaCBinaryTargetWriter::WriteModuleDepsSubstitution(
  ...
  if (target_->toolchain()->substitution_bits().used.count(substitution)) {
    ...
    target_group_.target_vars.push_back({substitution->ninja_name, val.str()});
  }
}
...
void NinjaCBinaryTargetWriter::WriteGCCPCHCommand(
  ...
  target_group_.edges.back().edge_vars.push_back(
      {flag_type->ninja_name, flag_val.str()});
}
...
void NinjaCBinaryTargetWriter::WriteSourceSetStamp(
  ...
  link_edge.blank_line_before = true;
  link_edge.outputs.push_back(link_phony);
  link_edge.rule = BuiltinTool::kBuiltinToolPhony;
  link_edge.explicit_inputs = link_files;
  link_edge.order_only_inputs = order_only_deps;
  ...
  default_edge.outputs.push_back(target_->dependency_output());
  default_edge.rule = BuiltinTool::kBuiltinToolPhony;
  default_edge.explicit_inputs.push_back(link_phony);
...
void NinjaCBinaryTargetWriter::WriteLinkerStuff(
  ...
  edge.rule = rule_prefix_ + tool_->name();

  ClassifiedDeps classified_deps = GetClassifiedDeps();

  // Object files.
  edge.explicit_inputs = object_files;
  ...
  if (target_->output_type() == Target::EXECUTABLE ||
      ...
      target_->output_type() == Target::LOADABLE_MODULE) {
    {
      ...
      edge.edge_vars.push_back({"ldflags", ss.str()});
    }
    {
      std::ostringstream ss;
      WriteLibs(ss, tool_);
      edge.edge_vars.push_back({"libs", ss.str()});
    }
    {
      std::ostringstream ss;
      WriteFrameworks(ss, tool_);
      edge.edge_vars.push_back({"frameworks", ss.str()});
    }
    {
      std::ostringstream ss;
      WriteSwiftModules(ss, tool_, swiftmodules);
      edge.edge_vars.push_back({"swiftmodules", ss.str()});
    }
    ...
    RecursiveTargetConfigStringsToStream(kRecursiveWriterKeepDuplicates,
                                         ...
                                         GetFlagOptions(), ss);
    edge.edge_vars.push_back({"arflags", ss.str()});
  }
  ...
  edge.edge_vars.push_back(
      {"output_dir", output_dir.empty() ? "" : " " + output_dir});
  ...
  if (!solibs.empty()) {
    ...
    edge.edge_vars.push_back({"solibs", ss.str()});
  }
  ...
  if (!transitive_rustlibs.empty()) {
    ...
    edge.edge_vars.push_back({"rlibs", ss.str()});
  }
