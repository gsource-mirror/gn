...
struct NinjaBuildEdge {
  ...
  // Whether to emit a blank line before this edge.
  bool blank_line_before = false;
};
...
struct NinjaTargetGroup {
  ...
  // Variables shared across all edges in this target (e.g. cflags, defines, target_out_dir).
...
struct NinjaFile {
  ...
  void Serialize(StringOutputBuffer& out) const;
};
