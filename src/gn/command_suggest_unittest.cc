...
TEST(Suggest, ResolveModuleName) {
...

  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(
        ...
        "my_module");
    std::vector<std::pair<const Target*, commands::PublicOrPrivate>> expected =
        {
            {&target, commands::PUBLIC},
        };
...
TEST(Suggest, ResolveTargetName) {
  ...
  std::vector<std::pair<const Target*, commands::PublicOrPrivate>>
      expected_label = {
          {&target, commands::PUBLIC},
      };
...
TEST(Suggest, ResolveFileName) {
...

  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(
        ...
        "//public.h");
    std::vector<std::pair<const Target*, commands::PublicOrPrivate>> expected =
        {{&explicit_target, commands::PUBLIC}};
    EXPECT_TRUE(ok);
    EXPECT_EQ(expected, results);
  }

  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(
        ...
        "../../private.h");
    std::vector<std::pair<const Target*, commands::PublicOrPrivate>> expected =
        {{&explicit_target, commands::PRIVATE}};
    EXPECT_TRUE(ok);
    EXPECT_EQ(expected, results);
  }

  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(
        ...
        "implicit_public.h");
    std::vector<std::pair<const Target*, commands::PublicOrPrivate>> expected =
        {{&implicit_target, commands::PUBLIC}};
    EXPECT_TRUE(ok);
    EXPECT_EQ(expected, results);
  }
...

  {
    ...
    std::vector<std::pair<const Target*, commands::PublicOrPrivate>>
        expected_targets = {
            {&simple_secondary, commands::PUBLIC},
            {&simple_default, commands::PUBLIC},
        };
    EXPECT_TRUE(ok);
    EXPECT_EQ(expected_targets, results);
  }

  {
    ...
    std::vector<std::pair<const Target*, commands::PublicOrPrivate>>
        expected_targets = {{{&simple_secondary, commands::PUBLIC}}};
    EXPECT_TRUE(ok);
    EXPECT_EQ(expected_targets, results);
  }
}
