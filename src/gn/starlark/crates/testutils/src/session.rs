...
impl FakeSession {
    ...
    pub fn default_target(&self) -> FakeTargetRef {
        self.get_target(LabelRef::new(PackageRef::root(), "default"), self.default_toolchain.as_ref())
    }
    ...
    pub fn insert_target(&self, target: FakeTarget) -> FakeTargetRef {
        ...
        assert!(targets.insert(target_ref.clone()), "Inserting an already existing target into the map");
        target_ref
    }
