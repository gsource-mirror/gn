...
StringOutputBuffer JSONProjectWriter::GenerateJSON(
  ...
  // ResolvedTargetData is safe for concurrent access (the parallel Ninja writers
  // share a single instance the same way), so the per-target descriptions below
  // are rendered on a worker pool.
