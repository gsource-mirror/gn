...
load("//:rules.scl", "nonexistent")
    ...
    // Verify failure when calling Starlark function with insufficient arguments.
    {
      TestParseInput fail_input(R"gn(
load("//:rules.scl", "sum")
sum(1, 2)
...
sum(1, 2, 3) {
  d = 4
      ...
      ASSERT_TRUE(err.has_error());
      EXPECT_EQ(
          "ERROR Found `d` extra named parameter(s) for call to //:rules.scl.sum\n"
          "See //test:3:1: whence 'sum' was called.\n"
          "sum(1, 2, 3) {\n"
          "^-------------\n",
          err.to_string());
...
sum_wrapper(1, 2, 3) {
  d = 4
      ...
      ASSERT_TRUE(err.has_error());
      EXPECT_EQ(
          "ERROR at //:rules.scl:15:10: Found `d` extra named parameter(s) for call to //:rules.scl.sum\n"
          "  return sum(*args, **kwargs)\n"
          "         ^-------------------\n"
