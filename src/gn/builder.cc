...
bool Builder::ResolveGeneratedModulemaps(Err* err) {
  std::unordered_set<Target*> unvisited;
  for (auto& record : records_) {
    if (record.type() == BuilderRecord::ITEM_TARGET && record.item()) {
      unvisited.insert(record.item()->AsTarget());
    }
  }
  ...
  std::vector<Target*> unvisited_copy(unvisited.begin(), unvisited.end());
  for (Target* target : unvisited_copy) {
    visit(target);
  }
  ...
  auto GetImplicitTextualDep = [](const Target* target) -> const Target* {
    for (const auto& dep : target->GetDeps(Target::DEPS_LINKED)) {
      ...
      if (t->module_type().implicitly_textual()) {
        return t;
      }
    }
    return nullptr;
