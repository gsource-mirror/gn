...
ResolvedTargetData::TargetInfo* ResolvedTargetData::GetTargetInfo(
  ...
  // to this ResolvedTargetData instance. If it matches, the pointer is valid
  // and we can return it directly without taking locks or computing hashes.
...
ResolvedTargetData::TargetInfo* ResolvedTargetData::GetTargetInfoSlow(
  ...
  // concurrently, so serialize under infos_mutex_ and double-check the owner.
  ...
  // Store the info pointer first, then publish it by storing this instance as
  // the owner with memory_order_release. This ensures any subsequent load with
  // memory_order_acquire that sees 'owner == this' will observe the valid
  // result pointer.
