...
TEST(ResolvedTargetDataTest, PublicInputsInheritance) {
  ...
  EXPECT_TRUE(resolved.HasPublicInputs(&a));

  // B has A as a private dependency, so B itself does NOT have public_inputs for its dependents.
  EXPECT_FALSE(resolved.HasPublicInputs(&b));

  // C has A as a public dependency, so C has public_inputs to propagate.
  EXPECT_TRUE(resolved.HasPublicInputs(&c));

  // D has C as a private dependency, so D itself does NOT have public_inputs for its dependents.
  EXPECT_FALSE(resolved.HasPublicInputs(&d));

  // E has B as a private dependency, B has no public inputs, so E has none.
  EXPECT_FALSE(resolved.HasPublicInputs(&e));
}
