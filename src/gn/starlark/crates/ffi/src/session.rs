...
impl Session {
    ...
    fn load(&'static self, label: LabelRef<'_>) -> starlark::Result<FrozenModule> {
        self.loader
            .load(label, &self.path_resolver, &self.globals, &|pkg| {
                ...
                // eval context.
                crate::eval_context::EvalContext::new_bzl_file(self, unsafe { extend_lifetime(pkg) })
            })
    }
    ...
    pub fn load_values(
        ...
        let res = PackageRef::new(relative_to)
            .and_then(|pkg| Label::parse(label, pkg))
            .and_then(|label| self.load(label.as_ref()).map(|module| (label, module)))
            .and_then(|(label, module)| {
                for key in keys {
                    let value = module
                        .get(key)
