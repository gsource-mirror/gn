...
TEST(Suggest, ResolveModuleName) {
  ...
  {
    auto [results, ok] =
        commands::ResolveSuggestionToTarget(setup_scope.settings(), all_targets,
                                            default_toolchain, "my_module");
    std::vector<std::pair<const Target*, bool>> expected = {
        {&target, false},
    };
  ...
  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(setup_scope.settings(),
                                                  all_targets, default_toolchain,
                                                  "my_module_Private");
    std::vector<std::pair<const Target*, bool>> expected = {{&target, true}};
    EXPECT_EQ(expected, results);
    EXPECT_TRUE(ok);
  }
}
...
TEST(Suggest, ResolveTargetName) {
  ...
  std::vector<const Target*> all_targets;
  all_targets.push_back(&target);
  all_targets.push_back(&target_gcc);

  // Test resolving "//:hello"
  auto [results_label, ok_label] =
      commands::ResolveSuggestionToTarget(setup_scope.settings(), all_targets,
                                          setup_scope.toolchain()->label(),
                                          "//:hello");

  std::vector<std::pair<const Target*, bool>> expected_label = {
      {&target, false},
  };
  ...
  auto [results_toolchain, ok_toolchain] = commands::ResolveSuggestionToTarget(
      setup_scope.settings(), all_targets, default_toolchain,
      "//:hello(//build/toolchain:gcc)");

  std::vector<std::pair<const Target*, bool>> expected_toolchain = {
      {&target_gcc, false},
  };
...
TEST(Suggest, ResolveFileName) {
  ...
  setup_scope.build_settings()->SetRootPath(temp_dir.GetPath());

  base::WriteFile(temp_dir.GetPath().AppendASCII("public.h"), "", 0);
  base::WriteFile(temp_dir.GetPath().AppendASCII("private.h"), "", 0);
  base::WriteFile(temp_dir.GetPath().AppendASCII("implicit_public.h"), "", 0);
  base::WriteFile(temp_dir.GetPath().AppendASCII("no_target.h"), "", 0);
  base::WriteFile(temp_dir.GetPath().AppendASCII("simple.h"), "", 0);
  base::WriteFile(temp_dir.GetPath().AppendASCII("default_toolchain.h"), "", 0);
  base::WriteFile(temp_dir.GetPath().AppendASCII("secondary_toolchain.h"), "",
                  0);
  ...
  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(setup_scope.settings(), all_targets,
                                            current_toolchain, "//public.h");
    std::vector<std::pair<const Target*, bool>> expected = {
  ...
  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(setup_scope.settings(), all_targets,
                                            current_toolchain, "implicit_public.h");
    std::vector<std::pair<const Target*, bool>> expected = {
...

  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(
        setup_scope.settings(), all_targets, current_toolchain,
        "nonexistent_file.h");
    EXPECT_FALSE(ok);
  }
  ...
  {
    auto [results, ok] = commands::ResolveSuggestionToTarget(
        setup_scope.settings(), all_targets, current_toolchain, "no_target.h");
    std::vector<std::pair<const Target*, bool>> expected_targets;
...

  {
    ...
    std::vector<std::pair<const Target*, bool>> expected_targets = {{{&simple_secondary, false}}};
    EXPECT_EQ(expected_targets, results);
    EXPECT_TRUE(ok);
  }
}
