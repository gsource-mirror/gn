...
impl<T: NonOpaque> Slice<T> {
    ...
    pub fn as_slice(&mut self) -> &mut [T] {
        if self.raw.len == 0 {
            ...
            // (If T is opaque, rust is lied to and thinks a T is a u8, but &T remains correct).
...
impl<T> Drop for OwnedSlice<T> {
    ...
    fn drop(&mut self) {
        ...
        // Safety: Calling free is safe. The pointer is guarunteed to be valid and owned by us.
        unsafe {
