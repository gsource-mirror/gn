...
#define EXPECT_SUCCESS(err)                                            \
  ...
  if (const auto& test_err = (err); !test_err.has_error())            \
    ...
    TEST_NONFATAL_FAILURE_(::testing::TestResult(false, "EXPECT_SUCCESS(" #err ")")) << test_err.message()
...
#define ASSERT_SUCCESS(err)                                            \
  ...
  if (const auto& test_err = (err); !test_err.has_error())            \
    ...
    TEST_FATAL_FAILURE_(::testing::TestResult(false, "ASSERT_SUCCESS(" #err ")")) << test_err.message()
