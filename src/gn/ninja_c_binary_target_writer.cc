...
void NinjaCBinaryTargetWriter::WriteSources(
  ...
  for (const auto& source : target_->sources()) {
    ...
    // Ensure all outputs produced by this source are tracked for dependencies
    // (e.g., included in the target's stamp file), except those already
    // added to object_files.
    for (const auto& output : tool_outputs) {
      if (std::find(object_files->begin(), object_files->end(), output) ==
          object_files->end()) {
