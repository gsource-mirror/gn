...
toolchain("default") {
  ...
  TestProject(std::string_view build_gn)
      : TestProject(std::map<SourceFile, std::string>{
            {SourceFile("//BUILD.gn"), std::string(build_gn)}}) {}
...
TEST_F(SuggestTest, OutputSuggestions) {
  ...
  EXPECT_EQ(
      ...
      "//:private_target\n"
      "Suggestion: Move \"private_target_Private\" from `sources` to `public` "
...
source_set("included") {
  ...
  bool success = commands::OutputSuggestions(
