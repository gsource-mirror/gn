...
void NinjaCreateBundleTargetWriter::GenerateRules() {
  ...
  NinjaBuildEdge bundle_dir_edge;
  bundle_dir_edge.outputs.push_back(
      OutputFile(settings_->build_settings(),
                 target_->bundle_data().GetBundleRootDirOutput(settings_)));
  bundle_dir_edge.rule = BuiltinTool::kBuiltinToolPhony;
  bundle_dir_edge.explicit_inputs.push_back(target_->dependency_output());
  AddEdge(std::move(bundle_dir_edge));
}
...
void NinjaCreateBundleTargetWriter::WriteCopyBundleFileRuleSteps(
  ...
  for (const SourceFile& source_file : file_rule.sources()) {
    ...
    NinjaBuildEdge edge;
    edge.outputs.push_back(expanded_output_file);
    edge.rule = GetNinjaRulePrefixForToolchain(settings_) +
                GeneralTool::kGeneralToolCopyBundleData;
    edge.explicit_inputs.push_back(
        OutputFile(settings_->build_settings(), source_file));
    edge.implicit_inputs = implicit_deps;
...
void NinjaCreateBundleTargetWriter::WriteCompileAssetsCatalogStep(
  ...
  if (target_->bundle_data().assets_catalog_sources().empty()) {
    ...
    NinjaBuildEdge edge;
    edge.outputs.push_back(partial_info_plist);
    edge.rule = GetNinjaRulePrefixForToolchain(settings_) +
                GeneralTool::kGeneralToolStamp;
    edge.implicit_inputs = implicit_deps;
    edge.order_only_inputs = order_only_deps;
  ...
  NinjaBuildEdge edge;
  edge.outputs.push_back(compiled_catalog);
  if (partial_info_plist != OutputFile()) {
    ...
    edge.implicit_outputs.push_back(partial_info_plist);
  }
  ...
  edge.implicit_inputs.push_back(input_dep);
  edge.implicit_inputs.insert(edge.implicit_inputs.end(), implicit_deps.begin(),
                              implicit_deps.end());
  edge.order_only_inputs = order_only_deps;

  edge.edge_vars.push_back(
      {"product_type", target_->bundle_data().product_type()});

  if (partial_info_plist != OutputFile()) {
    std::ostringstream ss;
    path_output_.WriteFile(ss, partial_info_plist);
    edge.edge_vars.push_back({"partial_info_plist", ss.str()});
  }
  ...
  if (!flags.empty()) {
    ...
    edge.edge_vars.push_back(
        {SubstitutionXcassetsCompilerFlags.ninja_name, ss.str()});
  }
...
NinjaCreateBundleTargetWriter::WriteCompileAssetsCatalogInputDepsStampOrPhony(
  ...
  NinjaBuildEdge edge;
  edge.outputs.push_back(xcassets_input_stamp_or_phony);
  edge.rule = tool;

  for (const Target* target : dependencies) {
    if (target->has_dependency_output()) {
      edge.explicit_inputs.push_back(target->dependency_output());
    }
  }
  AddEdge(std::move(edge));
...
void NinjaCreateBundleTargetWriter::WritePostProcessingStep(
  ...
  *output_files = post_processing_output_files;
  ...
  NinjaBuildEdge edge;
  edge.outputs = std::move(post_processing_output_files);
  edge.rule = post_processing_rule_name;
  edge.implicit_inputs.push_back(post_processing_input_stamp_file);
  AddEdge(std::move(edge));
}
...
NinjaCreateBundleTargetWriter::WritePostProcessingInputDepsStampOrPhony(
  ...
  NinjaBuildEdge edge;
  edge.outputs.push_back(stamp_or_phony);
  edge.rule = tool;
