...
std::string_view Target::get_extra_input() const {
  if (!starlark_target_) {
    return {};
  }
  const StarlarkSession& session = settings()->build_settings()->starlark_session();
  rust::Str rust_val = rust::get_extra_input(session.rust_session(), starlark_target_);
  return std::string_view(rust_val.data(), rust_val.size());
}
