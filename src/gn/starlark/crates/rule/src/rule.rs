    ...
    for (name, parent_schema) in parent_attrs {
        if let Some(child_schema) = child_attrs.get(name) {
            if !matches!(parent_schema.kind(), AttrKind::Label | AttrKind::LabelList) {
                return Err(
                    crate::errors::Error::OnlyLabelTypesMayBeOverridden(name.clone()).into(),
                );
            }
            ...
            {
                return Err(crate::errors::Error::AttributeTypeMismatch(name.clone()).into());
            }
        } else {
    ...
    pub fn new(
        ...
        for name in attrs.keys() {
            if RESERVED_ATTRS.contains(&name.as_str()) {
                return Err(crate::errors::Error::ReservedAttribute(name.clone()).into());
            }
        }
        ...
        let parent_schema = match parent {
            Some(parent_val) => {
                if let Some(rule) = parent_val.downcast_ref::<FrozenRule<C>>() {
                    ...
                    return Err(crate::errors::Error::ParentMustBeARule.into());
                }
            },
            None => None,
        };
