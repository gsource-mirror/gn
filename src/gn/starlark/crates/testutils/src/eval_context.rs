...
impl FakeEvalContext {
    ...
    pub fn default(session: Rc<FakeSession>) -> Self {
...
impl EvalContextAttrExt for FakeEvalContext {
    fn create_target(
        ...
        let target = self.session.insert_target(FakeTarget {
            label,
            toolchain,
            ...
            outputs: vec![],
            attrs,
            dependencies: std::cell::RefCell::new(std::collections::HashSet::new()),
        });
        Ok(target)
    }
}
