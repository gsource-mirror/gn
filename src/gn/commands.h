...
// Caches the mapping of SourceFile -> target(s) to accelerate suggestion resolution.
class TargetResolutionCache {
 public:
  TargetResolutionCache();
  ~TargetResolutionCache();

  // Returns pointer to vector of (Target*, ApiScope) for the given file,
  // or nullptr if file is not in any target.
