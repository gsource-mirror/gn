  ...
  for (ConfigValuesIterator iter(target_); !iter.done(); iter.Next()) {
    for (const auto& pattern : iter.cur().additional_outputs()) {
      ...
      if (seen_patterns.insert(pattern_str).second) {
        all_additional_outputs.push_back(pattern);
      }
    }
  }
  ...
  for (const auto& source : target_->sources()) {
    ...
    if (tool_name != Tool::kToolNone) {
      ...
      for (const auto& pattern : all_additional_outputs) {
        OutputFile extra_output = SubstitutionWriter::ApplyPatternToSourceAsOutputFile(
            target_, settings_, pattern, source);
        if (std::find(tool_outputs.begin(), tool_outputs.end(), extra_output) == tool_outputs.end()) {
          tool_outputs.push_back(extra_output);
        }
      }
