...
void NinjaTargetWriter::AddTargetVar(std::string_view name, std::string value) {
  target_group_.target_vars.push_back({name, std::move(value)});
}
...
void NinjaTargetWriter::Run() {
  NinjaFile file;
  file.AddTargetGroup(GenerateTargetGroup());
  file.Hoist();
  file.Serialize(out_);
}
...
void NinjaTargetWriter::WriteEscapedSubstitution(const Substitution* type) {
  ...
  target_group_.target_vars.push_back({type->ninja_name, val.str()});
}
...
void NinjaTargetWriter::WriteCCompilerVars(
  ...
  if (bits.used.count(&CSubstitutionDefines)) {
    ...
    RecursiveTargetConfigToStream<std::string>(kRecursiveWriterSkipDuplicates,
                                               ...
                                               DefineWriter(), val);
    target_vars.push_back({CSubstitutionDefines.ninja_name, val.str()});
  }
  ...
  if (bits.used.count(&CSubstitutionFrameworkDirs)) {
    ...
    RecursiveTargetConfigToStream<SourceDir>(
        ...
        val);
    target_vars.push_back({CSubstitutionFrameworkDirs.ninja_name, val.str()});
  }
  ...
  if (bits.used.count(&CSubstitutionIncludeDirs)) {
    ...
    target_vars.push_back({CSubstitutionIncludeDirs.ninja_name, val.str()});
  }
  ...
  auto write_flag = [&](const Substitution* subst, bool has_pch,
    ...
    target_vars.push_back({subst->ninja_name, val.str()});
  };
  ...
  if (target_->source_types_used().SwiftSourceUsed() || !respect_source_used) {
    if (bits.used.count(&CSubstitutionSwiftModuleName)) {
      ...
      target_vars.push_back(
          {CSubstitutionSwiftModuleName.ninja_name, val.str()});
    }
    ...
    if (bits.used.count(&CSubstitutionSwiftBridgeHeader)) {
      ...
      target_vars.push_back(
          {CSubstitutionSwiftBridgeHeader.ninja_name, val.str()});
    }
    ...
    if (bits.used.count(&CSubstitutionSwiftModuleDirs)) {
      ...
      target_vars.push_back(
          {CSubstitutionSwiftModuleDirs.ninja_name, val.str()});
    }
  }
}
...
void NinjaTargetWriter::WriteRustCompilerVars(
  ...
  auto write_flag = [&](const Substitution* subst,
                        const std::vector<std::string>& (ConfigValues::*getter)()
                            const) {
    std::ostringstream val;
...
NinjaTargetWriter::WriteInputDepsStampOrPhonyAndGetDep(
  ...
  NinjaBuildEdge edge;
  edge.outputs.push_back(input_stamp_or_phony);
  edge.rule = tool;
  edge.explicit_inputs = deps.implicit;
  edge.order_only_inputs = deps.order_only;
...
void NinjaTargetWriter::WriteStampOrPhonyForTarget(
  ...
  if (target_->has_dependency_output_file()) {
    ...
    edge.outputs.push_back(stamp_file);
    ...
    edge.outputs.push_back(phony_target);

    edge.rule = BuiltinTool::kBuiltinToolPhony;
  } else {
  ...
  edge.explicit_inputs = std::move(all_files);
...
void NinjaTargetWriter::WriteValidations() {
  ...
  for (const auto& pair : validations) {
    ...
    out_ << " ";
    path_output_.WriteFile(out_, pair.ptr->dependency_output());
  }
}
