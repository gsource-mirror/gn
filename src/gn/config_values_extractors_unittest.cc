...
TEST(ConfigValuesGenerator, AdditionalOutputs) {
  ...
  outputs_value.list_value().push_back(
      Value(nullptr, "//out/Default/{{source_name_part}}.dwo"));
  setup.scope()->SetValue("additional_outputs", outputs_value, nullptr);
  ...
  ASSERT_EQ(config_values.additional_outputs().size(), 1u);
  EXPECT_EQ(config_values.additional_outputs()[0].AsString(),
            "//out/Default/{{source_name_part}}.dwo");
}
