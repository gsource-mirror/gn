...
class BuilderRecord {
 public:
...
#if DEBUG_BUILDER_RECORD
  const char* ToDebugCString(RecordState);
#endif  // DEBUG_BUILDER_RECORD
  ...
  // Notiyfy all dependents that the current record's item state
  // has changed. |update_state()| will be called for any
  ...
  template <typename UPDATE_DEPENDENT>
  bool NotifyDependentsOfStateChange(RecordState from_state,
    ...
    if (from_state != state_) {
      for (auto& pair : waiting_map_) {
        BuilderRecord* dependent = pair.first;
        if (dependent->OnDepStateChange(this, from_state, state_, pair.second))
