...
// A NodeRef represents a reference to a node within a tree, storing its
// full ancestor path from the root. It must always point to a valid node.
class NodeRef {
 public:
  explicit NodeRef(std::vector<ParseNode*> stack) : stack_(std::move(stack)) {
    DCHECK(!stack_.empty());
  }
...
void FindNodesRecursive(
    ...
    const std::function<std::optional<T>(NodeRef&)>& transform,
...
class BuildFile {
 public:
  ...
  // Parses a user-provided value and transfers ownership of the ParseNode.
  // The backing InputFile is stored in extra_files_ to keep the string alive.
 ...
 private:
  BuildFile(SourceFile source_file,
