...
void AddCrateVar(std::string_view name,
                 const std::string& value,
                 EscapeOptions opts,
                 std::vector<NinjaVariable>& target_vars) {
  std::ostringstream ss;
  EscapeStringToStream(ss, value, opts);
  target_vars.push_back({name, ss.str()});
}

void AddCrateVars(const Target* target,
                  const Tool* tool,
                  EscapeOptions opts,
                  std::vector<NinjaVariable>& target_vars) {
  AddCrateVar(kRustSubstitutionCrateName.ninja_name,
  ...
  AddCrateVar(kRustSubstitutionCrateType.ninja_name, crate_type, opts,
              target_vars);

  AddCrateVar(SubstitutionOutputExtension.ninja_name,
              SubstitutionWriter::GetLinkerSubstitution(
                  target, tool, &SubstitutionOutputExtension),
              opts, target_vars);
  AddCrateVar(SubstitutionOutputDir.ninja_name,
...
void NinjaRustBinaryTargetWriter::WriteCompilerVars() {
  ...
  AddCrateVars(target_, tool_, opts, target_group_.target_vars);
...
void NinjaRustBinaryTargetWriter::WriteSourcesAndInputs() {
  ...
  target_group_.edges.back().edge_vars.push_back({"sources", val.str()});
}
...
void NinjaRustBinaryTargetWriter::WriteExternsAndDeps(
  ...
  target_group_.edges.back().edge_vars.push_back({"externs", externs_val.str()});
  ...
  target_group_.edges.back().edge_vars.push_back(
      {"rustdeps", rustdeps_val.str()});
  ...
  target_group_.edges.back().edge_vars.push_back({"ldflags", ldflags_val.str()});
}
