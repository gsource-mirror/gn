...
void NinjaTargetWriter::WritePublicInputsStampOrPhony(
    ...
    ResolvedTargetData* resolved,
    std::ostream& out) {
  if (!resolved->HasPublicInputs(target))
    return;
  ...
  for (const auto& dep : target->public_deps()) {
    if (resolved->HasPublicInputs(dep.ptr)) {
...
NinjaTargetWriter::WriteInputDepsStampOrPhonyAndGetDep(
  ...
  for (const auto& pair : target_->public_deps()) {
    if (resolved().HasPublicInputs(pair.ptr)) {
      public_inputs_deps.push_back(GetPublicInputsOutputFile(pair.ptr, settings_->build_settings()));
    }
  }
  ...
  public_inputs_deps.erase(std::unique(public_inputs_deps.begin(), public_inputs_deps.end()), public_inputs_deps.end());
  deps.implicit.insert(deps.implicit.end(), public_inputs_deps.begin(), public_inputs_deps.end());
