...
use starlark::values::{list::UnpackList, Heap, Value, ValueLike as _, UnpackValue};
use types::File;

use crate::{Depset, Kind, Order, UnpackDepset};

/// Inner implementation of the Starlark `depset(...)` constructor.
/// This checks type matching, detects order conflicts, and creates the depset value.
pub fn depset_constructor<'v, C: types::EvalContext>(
    ...
    if let Some(transitive_list) = transitive {
        for child_depset in transitive_list.items {
            ...
            if child_order != Order::Unspecified {
                if order == Order::Unspecified {
                    ...
                    return Err(crate::Error::ConflictingOrders { order, child_order }.into());
                }
            }

            set_kind(*child_depset.kind())?;
            transitive_vec.push(child_depset.value());
        }
    }
    ...
    if direct_vec.is_empty() && transitive_vec.len() == 1 {
        ...
        let phony = if kind == Kind::File {
            if direct_vec.len() == 1 && transitive_vec.is_empty() {
                // If the depset contains only a single element, its phony is actually just the real object.
                // We don't need to do this for transitive, because we optimize a depset with 1 transitive and no direct to not even create a depset object.
