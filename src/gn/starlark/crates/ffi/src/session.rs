...
impl Session {
    ...
    pub(crate) fn register_target(&self, target: crate::target::Target) -> crate::TargetRef {
        let label = target.ffi.label().as_ref().to_owned();
        let settings = unsafe { &*target.ffi.settings() };
        let toolchain = settings.toolchain_label().as_ref().to_owned();

        let pinned = Box::pin(target);
        // Safety: The reference lasts as long as the session, which is static.
