...
bool AddToolchainSources(
  ...
  const auto* targets = cache.GetTargetsForFile(file, all_targets);
  if (!targets) {
    return false;
  }
...
bool FileExists(const std::vector<const Target*>& all_targets,
  ...
  if (build_dir_path.IsParent(file_path)) {
    ...
    if (const auto* targets = cache.GetTargetsForFile(file, all_targets)) {
      for (const auto& [target, scope] : *targets) {
...
TargetResolutionCache::GetTargetsForFile(
    const SourceFile& file,
    const std::vector<const Target*>& all_targets) {
  if (!file_to_targets_.has_value()) {
    file_to_targets_.emplace();

    for (const Target* target : all_targets) {
      for (const auto& source : target->sources()) {
        if (auto dep_kind = DepKind(target, source); dep_kind.has_value()) {
          (*file_to_targets_)[source].emplace_back(target, *dep_kind);
        }
      }
      for (const auto& header : target->public_headers()) {
        auto& vec = (*file_to_targets_)[header];
        if (std::ranges::none_of(vec, [target](const auto& pair) {
              return pair.first == target;
            })) {
          if (auto dep_kind = DepKind(target, header); dep_kind.has_value()) {
            vec.emplace_back(target, *dep_kind);
          }
        }
      }
      for (const auto& output : target->computed_outputs()) {
        ...
        auto& vec = (*file_to_targets_)[source];
        if (std::ranges::none_of(vec, [target](const auto& pair) {
              return pair.first == target;
            })) {
          if (auto dep_kind = DepKind(target, source); dep_kind.has_value()) {
...
ResolveSuggestionToTarget(const BuildSettings* build_settings,
  ...
  if (!AddToolchainSources(all_targets, &current_toolchain, file, cache, results)) {
...
SuggestResult OutputSuggestions(const std::vector<const Target*>& all_targets,
  ...
  auto ResolveSuggestion = [&](std::string_view value,
    ...
    const auto& [targets, ok] = ResolveSuggestionToTarget(
        build_settings, all_targets, current_toolchain, value, cache, target_context);
    if (!ok) {
      StartError();
  ...
  if (labels_without_toolchain.size() == 1 &&
    ...
    OutputString("\n");
    SourceFile file =
        ResolveFilePath(build_settings, all_targets, included_name, cache, includer);
