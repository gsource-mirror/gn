...
TEST(Suggest, Visibility) {
  ...
  auto set_visibility = [&](Target* target, const std::string& pattern) {
    Value visibility_value(nullptr, Value::LIST);
    visibility_value.list_value().push_back(Value(nullptr, pattern));
    Err err;
    EXPECT_TRUE(
        target->visibility().Set(SourceDir("//"), "", visibility_value, &err));
  };

  auto create_target = [&](const std::string& name, Target::OutputType type,
                           auto fn) {
  ...
  EXPECT_EQ(
      ...
      "are visible to //:includer (defined at //BUILD.gn:1)\n"
      "Suggestion: Carefully consider whether you want to change the "
  ...
  EXPECT_EQ(
      ...
      "targets is visible to //:includer (defined at //BUILD.gn:1)\n"
      "Suggestion: Depend on one of the following:\n"
      "* //:exposer_visible\n"
      "* //:exposer_visible2\n",
      run_suggest(*invisible));
