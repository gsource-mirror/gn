...
pub struct Ctx<'v, C: EvalContextAttrExt> {
    ...
    #[allocative(skip)]
    rule_stack: RefCell<Vec<&'static FrozenRule<C>>>,
}

impl<'v, C: EvalContextAttrExt> Ctx<'v, C> {
    pub fn new(attrs: CtxAttr<'v>, rule: &'static FrozenRule<C>) -> Self {
        Self {
            attrs,
    ...
    pub fn run_super(
        ...
        let parent = {
            ...
            let current = rule_stack.last().unwrap();
            current
                .parent
    ...
    <C::Session as Session>::TargetRef: TargetAttrExt,
{
    ...
    fn freeze(self, _packer: &Freezer) -> FreezeResult<Self::Frozen> {
        Err(crate::errors::Error::CtxUnfreezable.into())
    }
}
