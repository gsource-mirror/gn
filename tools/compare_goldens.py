...
def main():
    parser = argparse.ArgumentParser(description='Compare or update ninja output files.')
    parser.add_argument('--update', action='store_true', help='Update the golden files')
    ...
    for f in sorted(all_files):
        ...
        if args.update:
            ...
            res = subprocess.run(['diff', '--color=always', '-u', str(want), str(got)])
            if res.returncode:
                return_code = 1

    if return_code:
        print(f"Run `{sys.executable} {Path(__file__).resolve()} {want_dir} {got_dir} --update` to update the golden files if the changes are intentional.")
