...
impl Session {
    ...
    pub(crate) fn register_target(&self, target: Target) -> crate::TargetRef {
        ...
        let static_ref: &'static Target = match targets.entry((label, toolchain)) {
            Entry::Vacant(e) => {
    ...
    pub(crate) fn register_cxx_target(&self, cxx: &crate::bridge::CxxTarget) -> &Target {
        self.register_target(Target {
            // Safety: Target pointers in GN live for the static session.
            cxx: unsafe { types::util::extend_lifetime(cxx) },
            starlark: None,
        })
        .0
    }
