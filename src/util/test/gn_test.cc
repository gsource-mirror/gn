...
std::string DiffStrings(std::string_view expected, std::string_view actual) {
  ...
  auto fallback = [&]() {
    return base::StringPrintf("Expected:\n%s\nActual:\n%s", expected.data(), actual.data());
  };
  ...
  if (base::WriteFile(expected_path, actual.data(), actual.size()) !=
      static_cast<int>(actual.size())) {
    return fallback();
  }
  if (base::WriteFile(actual_path, expected.data(), expected.size()) !=
