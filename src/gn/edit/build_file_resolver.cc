...
std::unique_ptr<ParseNode> BuildFile::parse(std::string_view value) {
  // Generated nodes in an AST don't have any line numbers associated with them.
  ...
  // LiteralNode takes StringAtom which lives forever, so no file needed here.
...
std::vector<EditTarget> BuildFile::targets() {
  return FindNodes<EditTarget>(
      tree_root_.get(), [this](NodeRef& node_ref) -> std::optional<EditTarget> {
        if (auto* func = node_ref.node()->AsFunctionCall()) {
          if (func->block() && func->args() &&
            ...
            if (auto name =
              ...
              if (label_matcher_.matches(*name)) {
                EditTarget target;
                target.name = *name;
                target.parent = node_ref.parent();
                target.block = node_ref.node()->AsFunctionCallMut()->block();
                return target;
              }
            }
          }
...
std::vector<Assignment> EditTarget::assignments(std::string_view attr) const {
  return FindNodes<Assignment>(
      block, [attr](NodeRef& node_ref) -> std::optional<Assignment> {
        if (auto* op = node_ref.node()->AsBinaryOp()) {
...
void NodeRef::add_todo(std::string_view message) const {
