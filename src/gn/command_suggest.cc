...
bool OutputSuggestions(const std::vector<const Target*>& all_targets,
  ...
  auto OutputDepSuggestion = [&](const Target* included) {
    OutputInsertionHint(
        dep_field,
  ...
  auto PrintCandidates = [&](std::vector<const Target*> candidates) {
  ...
  if (visible_candidates.size() == 1) {
    OutputDepSuggestion(visible_candidates.front());
    ...
    OutputString("\n");
    OutputSuggestion("Depend on one of the following:\n");
    PrintCandidates(visible_candidates);
    ...
    PrintCandidates(all_candidates);
  }
