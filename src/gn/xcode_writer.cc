...
std::set<base::FilePath> FindFilesInRoot(const base::FilePath& root,
                                         bool use_git) {
  if (use_git) {
    ...
    if (::internal::ExecProcess(cmdline, root, &std_out, &std_err, &exit)) {
      if (exit == 0) {
        std::vector<std::string_view> lines = base::SplitStringPiece(
            std_out, std::string_view("\0", 1), base::KEEP_WHITESPACE,
            base::SPLIT_WANT_NONEMPTY);
...
std::set<base::FilePath> FilterPathsMatchingPatterns(
    const std::set<base::FilePath>& paths,
    const std::vector<base::FilePath::StringType>& patterns) {
  std::set<base::FilePath> filtered_paths;
  ...
  if (!options_.additional_files_patterns.empty()) {
    ...
    for (const base::FilePath& root : roots) {
      g_scheduler->ScheduleWork([&]() {
        std::set<base::FilePath> local_paths = FilterPathsMatchingPatterns(
            FindFilesInRoot(root, use_git), patterns);

        std::lock_guard<std::mutex> l(lock);
        if (!local_paths.empty()) {
          for (const auto& path : local_paths) {
            collected_paths.insert(path);
          }
        }
