...
TEST(NinjaTargetWriter, PhonyPropagation) {
  ...
  // TODO: This comment will be updated to reflect the new behavior
  // (containing A's output) when the changes are relanded.
  ...
  // (phony/foo/a).
  {
    ...
    EXPECT_EQ(
        ...
        "phony/foo/a\n"
        "\n"
        "build phony/foo/b_pub: phony b_pub.out\n",
        stream.str());
  }
  ...
  // TODO: This comment will be updated to reflect the new behavior
  // (containing B_data) when the changes are relanded.
  // 6. Verify C_data's stamp/phony target. It should contain B_data, but NOT A.
  {
    ...
    EXPECT_EQ(
        ...
        "phony/foo/b_data\n"
        "\n"
        "build phony/foo/c_data: phony c_data.out\n",
        stream.str());
  }
