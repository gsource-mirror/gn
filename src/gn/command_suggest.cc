...
const char kSuggest_Help[] =
  ...
  Where each includer or included is either a label, a module name,
  or a file path relative to build directory or repo root.
...
// target.
std::optional<PublicOrPrivate> DepKind(const Target* target,
...
bool AddToolchainSources(
    ...
    const SourceFile& file,
    std::vector<std::pair<const Target*, PublicOrPrivate>>& results) {
...
std::pair<std::vector<std::pair<const Target*, PublicOrPrivate>>, bool>
ResolveSuggestionToTarget(const BuildSettings* build_settings,
  ...
  PublicOrPrivate is_private = PUBLIC;
  ...
  SourceFile file = ResolveFilePath(build_settings, std::string(input));
  if (file.is_null()) {
...
bool OutputSuggestions(const std::vector<const Target*>& all_targets,
  ...
  const char* dep_field = (dep_kind == PRIVATE) ? "deps" : "public_deps";
