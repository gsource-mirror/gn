...
TEST_F(NinjaCBinaryTargetWriterTest,
  ...
  // Verify B's output
  {
    ...
    const char expected[] =
        ...
        "build phony/foo/b: phony phony/foo/b.link obj/foo/b.dwo "
        "obj/foo/b2.dwo\n";
...
TEST_F(NinjaCBinaryTargetWriterTest, WinPrecompiledHeaders) {
  ...
  // This target specifies PCH.
  {
    ...
    const char pch_win_expected[] =
        ...
        "withpch/phony/foo/pch_target.link\n";
...
TEST_F(NinjaCBinaryTargetWriterTest, SwiftModule) {
...

  {
    ...
    const char expected[] =
        ...
        "build phony/foo/foo: phony phony/foo/foo.link gen/foo/foo.h "
  ...
  // Swift module_dirs correctly set if dependency between Swift modules.
  {
    ...
    const char expected[] =
        ...
        "build phony/bar/bar: phony phony/bar/bar.link gen/bar/bar.h "
  ...
  // even if the dependency is indirect (via public_deps).
  {
    ...
    const char expected[] =
        ...
        "  restat = 1\n"
        "\n"
        "build phony/bar/bar.link: phony obj/bar/bar.o || phony/bar/group "
        "phony/foo/foo.link\n"
        "build phony/bar/bar: phony phony/bar/bar.link gen/bar/bar.h "
        "obj/bar/Bar.swiftmodule\n";
