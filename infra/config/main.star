...
def ci_builder(name, os, ref, suffix, console_name, trigger_name, caches = None):
    builder(name, "ci", os, ref = ref, suffix = suffix, caches = caches, triggered_by = [trigger_name])
...
def try_builder(name, os, ref, suffix, cq_group_name, caches = None):
...
def setup_branch(ref, suffix, console_name, cq_group_name, console_title):
    # CI builders should only be enabled for the main branch, since they push
    # to CIPD.
    enable_ci = suffix == ""

    trigger_name = "gn-trigger" + suffix
    if enable_ci:
        luci.gitiles_poller(
            name = trigger_name,
            bucket = "ci",
        ...
        # Console view entries are only generated for CI builders.
        luci.console_view(
            name = console_name,
            title = console_title,
            repo = "https://gn.googlesource.com/gn",
