...
std::optional<int> GetAttributeOrder(std::string_view attribute) {
  ...
  if (attribute == "output_name" || attribute == "output_prefix_override" ||
    ...
    // 300 => config
             ...
             attribute.starts_with("rust") || attribute.ends_with("configs") ||
...
std::vector<std::optional<int>> GetOrders(
  ...
  std::vector<std::optional<int>> orders(nodes.size());
  std::optional<int> next_order = std::nullopt;
  for (size_t i = nodes.size(); i-- > 0;) {
    if (nodes[i]->AsBlockComment()) {
      orders[i] = next_order;
    } else {
      orders[i] = GetOrder(nodes[i].get());
      next_order = orders[i];
    }
  }
  return orders;
  ...
  if (const auto* op = node->AsBinaryOp()) {
    ...
    for (const auto& order : orders) {
      if (order && result && order != result) {
        // Multiple differing orders, so unsure.
        return std::nullopt;
      } else if (!result) {
        result = order;
      }
...
std::unique_ptr<ParseNode> SimplifyExpression(std::unique_ptr<ParseNode> expr) {
  // We could recurse into minuses, but `gn edit` never modifies anything in a minus, so no need to simplify.
