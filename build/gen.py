...
def main(argv):
  ...
  args_list.add('--gen', action='store', default=None,
                    metavar='SRC_DIR=OUT_DIR', dest='gen_target',
                    help=('Generate Ninja targets `gen` and `gen.json` to run gn gen and siso dump in an external repository. '
                          'Format: <src_dir>=<out_dir> (e.g. ~/chromium/src=out/Default)'))
...
def WriteGenericNinja(path, static_libraries, executables,
  ...
  if options.gen_target:
    if '=' not in options.gen_target:
      raise ValueError(f'Invalid --gen format: {options.gen_target!r}. Expected <src_dir>=<out_dir>')
    gen_src_dir, gen_out_dir = options.gen_target.split('=', 1)
    ninja.AddExternalGenTarget('gen', pathlib.Path(os.path.expanduser(gen_src_dir)).resolve(), pathlib.Path(gen_out_dir))
