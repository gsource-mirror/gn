...
It is invoked as a post-build step by the Ninja 'cargo' rule. It performs the
following tasks:
1. Locates the compiled artifacts (libraries or test binaries) from Cargo's
   profile directory.
2. Symlinks or copies the compiled artifacts to the requested Ninja output path.
3. For test targets: generates a Python test runner script that invokes all unit
   test binaries found in the workspace.
4. Translates and merges Cargo-generated `.d` dependency files (depfiles) into
...
def process_test_target(out_path: Path, cargo_out_dir: Path) -> list[Path]:
  ...
  for c in (cargo_out_dir / 'deps').iterdir():
    if c.is_file() and os.access(c, os.X_OK) and c.suffix not in ('.so', '.dylib', '.dll'):
      parts = c.name.split('-')
  ...
  return [b.parent / f'{b.name}.d' for b in newest_binaries]


def main():
  if len(sys.argv) != 4:
    print(
        'Usage: symlink_cargo_output.py <test|lib> <out> <cargo_out_dir>',
        file=sys.stderr,
    )
    sys.exit(1)
