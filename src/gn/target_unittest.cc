...
TEST_F(TargetTest, ModulemapInheritFallback) {
  ...
  EXPECT_TRUE(check.module_type().test(Target::MODULEMAP_DISALLOWED_NONTEXTUAL_DEP));

  // inherit target should be nontextual
  TestTarget inherit_nontextual(setup, "//:inherit_nontextual", Target::SOURCE_SET);
  ...
  EXPECT_TRUE(inherit_textual.module_type().test(Target::MODULEMAP_INHERITED_TEXTUAL));
  ...
  std::string expected_err =
      "To modularize targets, add `generate_modulemap = \"try|inherit|textual\"` (prefer \"try\") to all non-modularized targets it transitively depends on:\n"
      "//:try\n"
      "  //:inherit_textual\n"
