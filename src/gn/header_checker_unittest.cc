...
TEST_F(HeaderCheckerTest, CheckInclude) {
  ...
  errors.clear();
  checker->CheckInclude(a_cache, false, input_file, SourceFile("//random.h"), range,
                        &errors);
  EXPECT_EQ(errors.size(), 0);
...
TEST_F(HeaderCheckerTest, CheckIncludeSwiftModule) {
  ...
  errors.clear();
  checker->CheckInclude(d_cache, false, input_file, generated_header, range, &errors);
  EXPECT_GT(errors.size(), 0);

  // Check that unrelated target D cannot include S's bridge header.
  errors.clear();
  checker->CheckInclude(d_cache, false, input_file, bridge_header, range, &errors);
  EXPECT_GT(errors.size(), 0);
}
