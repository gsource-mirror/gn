...
///
/// Guaranteed to match the type of the corresponding `AttrSchema`.
/// It will be None iff the intended starlark representation is None.
/// For example, the bool type defaults to false, attr.bool() can never produce `Attr::None`.
/// On the other hand, `attr.label()` will produce either `Attr::None` or `Attr::Label`.
