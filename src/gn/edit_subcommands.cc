...
Result<ParseNodeGenerator> CreateParseNodeGenerator(
  ...
  if (kind == "expr") {
  ...
  } else if (kind == "list" || values.size() > 1) {
    ASSIGN_OR_RETURN(std::vector<Value> out, ParseValues(values));
    return [out = std::move(out)](BuildFile& build_file)
               -> Result<std::unique_ptr<ParseNode>> {
    ...
    return [val = std::move(val)](BuildFile& build_file)
               -> Result<std::unique_ptr<ParseNode>> {
...
// Sets an attribute using a parse node generator.
EditCommand SetCommand(std::string_view attribute,
                       ParseNodeGenerator generator) {
  return EditTargetCommand([attribute = std::string(attribute),
...
Result<EditCommand> ParseCommand(std::vector<std::string> args) {
  ...
  if (args[0] == "add") {
    ...
    return SetCommand(attr, std::move(generator));
