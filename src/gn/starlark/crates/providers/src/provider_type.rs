...
impl<'v> StarlarkValue<'v> for ProviderType {
    ...
    fn invoke(
        ...
        // This only happens in export_as, which can only be called while the ProviderType is not frozen.
        // Until it is frozen, evaluation is purely single-threaded.
        ...
        spec.parser(args, eval, |param_parser, eval| {
            let values: Box<[CoercableOption<Value<'v>>]> = (0..self.fields.len())
                .map(|_| param_parser.next_opt().map(|v| v.into()))
    ...
    fn export_as(&self, name: &str, eval: &mut Evaluator<'v, '_, '_>) -> starlark::Result<()> {
        ...
        // Safety: This is safe as long as there are no other borrows. This can only happen before freezing, when starlark is still executing single-threaded.
...
impl ProviderType {
    ...
    /// This provider is not yet configured, and is unusable until `export_as` is called.
    ...
    /// Creates a new builtin provider type.
    pub fn builtin_new(
        name: &'static str,
        fields: Vec<(&'static str, ParametersSpecParam<FrozenValue>)>,
        ...
        Self {
            id,
            data: UnsafeCell::new(Some(ProviderTypeData {
                name: name_frozen,
                parameter_spec: ParametersSpec::new_named_only(name, fields),
            })),
            fields: field_map,
        }
    }
...
p(a=1, b=2)
"#,
            "Found `b` extra named parameter(s) for call to p",
        );
        a.eq(
            r#"
p = provider(fields=['a'])
