...
struct ErrOutput {
  ...
  virtual void Write(std::string_view text, TextDecoration dec = DECORATION_NONE) = 0;
};
...
void Err::InternalFormat(ErrOutput& output, bool is_sub_err, bool is_fatal) const {
  DCHECK(info_);
