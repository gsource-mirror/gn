...
mod bridge {
    ...
    unsafe extern "C++" {
        ...
        type OutputFile;
        pub(crate) fn value(self: &OutputFile) -> string_view;

        type SourceDir;
        pub(crate) fn SourceWithNoTrailingSlash(self: &SourceDir) -> string_view;

        type Label;
        pub(crate) fn dir(self: &Label) -> &SourceDir;
        #[rust_name = "name_cxx"]
        pub(crate) fn name(self: &Label) -> &CxxString;

        type Settings;
        pub(crate) fn toolchain_label(self: &Settings) -> &Label;
        ...
        type TestWithScope;
        pub(crate) fn NewTestWithScope() -> UniquePtr<TestWithScope>;
        #[rust_name = "scope_cxx"]
        pub(crate) fn scope(self: Pin<&mut TestWithScope>) -> *mut Scope;
    }
}
