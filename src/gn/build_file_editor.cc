  ...
  for (const auto& target :
    ...
    for (const auto& assignment : target.assignments({"sources", "public"})) {
      for (const auto& element : FindAllListElements(assignment)) {
        ...
        auto condition = element.GetConditionString();
        auto it = source_conditions.find(source_file);
        if (it == source_conditions.end()) {
          source_conditions.emplace(source_file, std::move(condition));
