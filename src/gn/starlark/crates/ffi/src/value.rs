...
use crate::{
    bridge::{SliceAny, Value},
    CxxStringExt as _, Scope, Slice,
};

#[repr(u8)]
// We never construct value types, they're created via From<u8>
#[allow(dead_code)]
enum ValueType {
    None = 0,
    Boolean = 1,
...
impl From<u8> for ValueType {
    fn from(value: u8) -> Self {
        // Safety: the value should always come from C++'s Value object.
        unsafe { std::mem::transmute(value) }
    }
}

impl Value {
    fn list_value(&self) -> Slice<Self> {
        crate::bridge::list_value_cxx(self).into()
    }

    fn r#type(&self) -> ValueType {
        crate::bridge::type_cxx(self).into()
    }
    ...
    pub fn to_rust(&self, frozen_heap: &starlark::values::FrozenHeap) -> FrozenValue {
        match self.r#type() {
            ...
            ValueType::String => frozen_heap.alloc(self.string_value().as_str()),
            ValueType::List => {
                let slice = self.list_value();
                let mut items = Vec::new();
                for item in slice.iter() {
                    items.push(item.to_rust(frozen_heap));
                }
            ...
            ValueType::Scope => {
                ...
                if scope_ptr.is_null() {
                    ...
                    let items = scope.get_kv(frozen_heap);
                    frozen_heap.alloc(starlark::values::structs::AllocStruct(items))
                }
            },
        }
    }
    ...
    pub fn assign<'v>(
        ...
        if val.is_none() {
            ...
            let slice: Slice<Self> = SliceAny {
                ptr: unsafe { crate::bridge::SetValueList(self.as_mut(), origin, l.len()) }
                    .cast::<crate::bridge::Any>(),
                len: l.len(),
            }
            .into();
            for (el_pin, src) in slice.iter().zip(l.iter()) {
            ...
            for (v_starlark, v_cxx) in s.iter().map(|(_, v)| v).zip(values.as_slice()) {
