...
void NinjaTargetWriter::WriteStampOrPhonyForTarget(
  ...
  // Add dependency outputs of public_deps to this target's stamp or phony target
  // to propagate public dependencies transitively. This allows dependents of
  // this target to implicitly depend on the public_deps without listing them
  // all directly on their build lines, preventing inflation of implicit inputs.
  ...
  for (const auto& pair : target_->public_deps()) {
    if (pair.ptr->has_dependency_output()) {
      ...
      if (!std::ranges::contains(all_files, dep_out)) {
