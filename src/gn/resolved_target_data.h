...
class ResolvedTargetData {
 public:
  ...
  bool ExportsPublicInputs(const Target* target) const {
    return GetTargetExportsPublicInputs(target)->exports_public_inputs;
  }
 ...
 private:
  ...
  struct TargetInfo {
    ...
    std::atomic<bool> exports_public_inputs_resolved = false;
    ...
    bool exports_public_inputs = false;
  };
  ...
  const TargetInfo* GetTargetExportsPublicInputs(const Target* target) const {
    TargetInfo* info = GetTargetInfo(target);
    if (!info->exports_public_inputs_resolved.load(std::memory_order_acquire)) {
      std::lock_guard<std::mutex> lock(info->mutex);
      if (!info->exports_public_inputs_resolved.load(
              std::memory_order_relaxed)) {
        ComputeExportsPublicInputs(info);
        info->exports_public_inputs_resolved.store(true,
                                                   std::memory_order_release);
      }
    }
    return info;
  ...
  void ComputeExportsPublicInputs(TargetInfo* info) const;
